...
class EncoderImagePrecompAttn(nn.Module):
    ...
    def __init__(self, img_dim, embed_size, data_name, text_number, text_dim, use_abs=False, no_imgnorm=False):
        ...
        self.fc = nn.Linear(img_dim, embed_size)
        self.fc_CLIP = nn.Linear(img_dim, embed_size)############################################################################################################
        self.init_weights()
    ...
    def init_weights(self):
        ...
        self.fc.bias.data.fill_(0)
        ######################################################
        s = np.sqrt(6.) / np.sqrt(self.fc_CLIP.in_features +
                                  self.fc_CLIP.out_features)
        self.fc_CLIP.weight.data.uniform_(-s, s)
        self.fc_CLIP.bias.data.fill_(0)
    ...
    def forward(self, images, scene_text):
        ...
        img_emd_CLIP = self.fc_CLIP(images)
        img_emd_CLIP = l2norm(img_emd_CLIP)
        ######################################################################################
        
        
        ##########################################################################################################################
        image_only_CLIP=0
        if image_only_CLIP==1:
            device = "cuda:1" if torch.cuda.is_available() else "cpu"
            model_CLIP, preprocess = clip.load("ViT-B/32", device=device)
            images_clip = preprocess(images).unsqueeze(0).to(device)
            with torch.no_grad():
                image_features_CLIP = model_CLIP.encode_image(images_clip)
            image_features_CLIP = self.fc_CLIP(image_features_CLIP)
        ##########################################################################################################################
        image_features_CLIP = img_emd_CLIP######################################################################################
        ...
        rnn_img2, hidden_state2 = self.img_rnn(img_emd_CLIP)####################
        image_features_CLIP = hidden_state2[0]####################
        
        
        visual_features = visual_features + image_features_CLIP#############################################oneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneoneone
